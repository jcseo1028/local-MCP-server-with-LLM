...
- **IntentResolver**: 사용자 메시지를 분석하여 적절한 도구를 선택한다. LLM Connector를 직접 호출하되, 이는 "도구 실행"이 아닌 "라우팅 결정"이다. v2.1에서 **계획 수립**(최대 5개 항목) 역할도 포함한다. **v2.7**: "프로젝트 전체 구조 요약" 등 의도 시 `analyze_project_structure` 도구 선택 및 `multiFileContextRequired=true` 플래그 반환.
...
- **RunOrchestrator (v2.7 멀티파일 프로젝트 요약)**: `multiFileContextRequired=true` 의도 감지 시 context_collection 단계에서 `VectorSearchEngine.GetProjectStructureSummary()`를 호출해 프로젝트 전체 주요 파일의 대표 chunk를 수집한다. 최대 파일 수/chunk 수는 `Config.projectSummary.maxProjectFiles/maxProjectChunks` 제한 적용. 수집된 chunk들을 `BuildRagContext()`에서 멀티파일 format으로 조립해 LLM 입력에 전달한다. **SC-4**: 멀티파일 입력에서 `[FILE:]` 파싱 실패 시 단건 조용한 폴백 없이 1회 재시도 후 명시적 실패 처리. **SC-5**: `organize_imports` 결과는 using/import 외 본문 변경 여부를 검증하고, 본문이 변경된 경우 import 블록만 원본 본문에 자동 투영(자동 보정) 후 재검증한다. **v2.6**: `allowMultiToolPlan/maxPlanSteps` 기반 다중 도구 실행 계획(`PlanSteps`)을 구성하며, 코드 변경 단계는 `PendingPatch` 상태로 추적하고 `confirm/revert` API로 확정/되돌리기 상태 전이를 지원한다.
...
- **VectorSearchEngine 멀티파일 프로젝트 요약 (v2.7)**: `GetProjectStructureSummary(maxFiles, maxChunksPerFile)` 메서드 추가. 색인 대상 코드 파일 전체를 순회하면서 각 파일별 대표 chunk(class/region/method 단위)를 우선순위로 추출한다. 추출된 chunk를 `contracts.md` §4b ProjectStructureSummaryResponse 형식으로 반환해 RunOrchestrator의 멀티파일 context 구성에 사용된다. 파일이 많을 경우 Top-N 파일 우선 추출 방식으로 LLM 입력 한도 초과를 방지한다.
