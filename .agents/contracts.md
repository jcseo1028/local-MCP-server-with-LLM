...
```
Input {
  code: string | null           // 요약 대상 코드 텍스트 (단건 모드, files_context 없을 때 필수)
  ...
  files_context: string | null  // 멀티 파일 컨텍스트 블록 (RunOrchestrator.BuildFilesContext() 생성)
                                // files_context가 있으면 code/language 무시하고 멀티 파일 요약 모드로 동작
}
...
- **내부 처리**: `files_context` 있으면 → `code_section = files_context`, 없으면 → `code_section = "{language} 코드:\n```{language}\n{code}\n```"`
- **사용 모듈**: LLM Connector (모델 선택은 OllamaConnector에 위임 — summaryModel → defaultModel → 하드코딩 fallback)
- **프롬프트**: `prompts/summarize_current_code.prompt.md` — `{{code_section}}` 단일 변수, 구조화된 요약 (목적, 구성요소, 알고리즘, 주의사항)
