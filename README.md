...
프로젝트 전체 구조 요약 기능(v2.7.x)이 활성화되면, 요약 결과는 다음 경로에 자동 저장된다:
- **기본 경로**: `<solution-root>/docs/project-summary.md`
- **대체 경로**: `docs` 폴더가 없으면 `<solution-root>/.project-summary/project-summary.md` 사용
- **설정**: `appsettings.json`의 `projectSummary.outputPath`, `projectSummary.enabled`, `projectSummary.backupOldSummary` 옵션으로 커스터마이즈 가능
...
| `analyze_project_structure` | 프로젝트 전체 구조를 여러 파일의 주요 코드로 분석 (v2.7.x) | 🔄 스펙 확정 |
...
## 프로젝트 전체 구조 요약 (v2.7.x)

프로젝트 전체 구조 요약 기능은 여러 파일의 주요 코드를 멀티파일 chunk로 조립하여 LLM에 전달하고, 결과를 자동으로 문서 파일로 저장한다.

### 활성화 및 설정

`appsettings.json`에서 프로젝트 요약 설정:

```json
{
  "projectSummary": {
    "enabled": true,                                    // 기능 활성화 (기본 true)
    "outputPath": "docs/project-summary.md",            // 요약 결과 저장 경로 (기본값 사용 시 null)
    "maxProjectFiles": 50,                              // 멀티파일 요약 시 최대 파일 수 (기본 50)
    "maxProjectChunks": 3,                              // 파일당 최대 chunk 수 (기본 3)
    "backupOldSummary": true                            // 기존 요약 파일 백업 여부 (기본 true)
  }
}
```

- **outputPath 기본값**: `null`이면 `<solution-root>/docs/project-summary.md` 사용, `docs` 폴더 미존재 시 `<solution-root>/.project-summary/project-summary.md`로 자동 생성
- **backupOldSummary**: `true`이면 기존 파일을 `.bak` 확장자로 백업, `false`이면 덮어쓰기

### 사용 방법

VSIX 채팅 입력란에 다음과 같은 자연어 요청을 입력하면 analyze_project_structure 도구가 자동 선택되어 실행된다:
- "프로젝트 전체 구조를 요약해줘"
- "프로젝트 아키텍처를 분석해줘"
- "전체 프로젝트 문서를 작성해줘"

결과는 자동으로 지정된 경로(기본: `docs/project-summary.md`)에 저장되며, 이후 Markdown 문서로 참고 가능하다.
